...
class Traveler {
    ...
    static travelTo(creep, destination, options = {}) {
        ...
        if (!travelData.path) {
            ...
            if (state.cpu > REPORT_CPU_THRESHOLD) {
                ...
                console.log(`TRAVELER: heavy cpu use: ${creep.name}, cpu: ${state.cpu} origin: ${creep.pos}, dest: ${destination}, options:${JSON.stringify(options)}`);
            }
            let color = "orange";
            if (ret.incomplete) {
                // uncommenting this is a great way to diagnose creep behavior issues
                //console.log(`TRAVELER: incomplete path for ${creep.name}`);
                //console.log(`dest = ${destination} pos = ${creep.pos} range = ${options.range}`);
                color = "red";
            }
            if (options.returnData) {
        ...
        if (options.returnData) {
            if (nextDirection) {
                let nextPos = creep.pos.oneStep(nextDirection);
                if (nextPos) {
    ...
    static getStructureMatrix(room, freshMatrix) {
        let objectDestroyed = room.destroyedStructureEvents.length > 0;

        if (!this.structureMatrixCache[room.name] ||
            ((freshMatrix || objectDestroyed) && Game.time !== this.structureMatrixTick[room.name])) {
    ...
    static deserializeState(travelData, destination) {
        ...
        if (travelData.state) {
            ...
            state.cpu = 0; // hax! travelData.state[STATE_CPU];
...
Creep.prototype.travelTo2 = function (destination, options) {
    if (options && options.returnData) {
        this.logError('travelTo2 does not permit the use of returnData option.');
    }
    let defaultOptions = {
        roomCallback: DefaultRoomCallback
    };
    let mergedOptions = _.merge(defaultOptions, options);
    
    // Do this after the merge. Caller isn't allowed to override returnData.
    let returnData = {};
    mergedOptions.returnData = returnData;
    let result = Traveler.travelTo(this, destination, mergedOptions);
    this.nextPos = returnData.nextPos || this.pos;
    return result;
};

PowerCreep.prototype.travelTo = function (destination, options) {
    if (options && options.returnData) {
        this.logError('travelTo does not permit the use of returnData option.');
    }
    let defaultOptions = {
        roomCallback: DefaultRoomCallback,
        offRoad: true
